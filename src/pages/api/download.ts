...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    try {
        ...
        if (!reqImageUrl) {
            res.status(400).json({ error: true, msg: 'Image URL was not provided as a query parameter' });
            return;
        }
        ...
        res.setHeader('Content-Disposition', `attachment; filename=image.${imageType}`);
